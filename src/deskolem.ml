...
let elim_hypothesis :
    ...
    Console.out 4 "[DEBUG] fu : %a@." Print.pp_term fu;
        (* (z / fu) pb. *)
    ...
    let zen = Files.PathMap.find ["logic";"zen"] !Sign.loaded  in
    let ex_E = Sign.find zen "exists_E" in
    let applied_pa = Basics.add_args pa u in
    let pa_u_b_l = Basics.add_args (Symb ex_E) 
                     [iota; a; applied_pa; unProof sign b; z_lambda]
    in
      ...
      begin
        ...
        | None      ->
           let handle_apps head type_head args = 
             ...
             Console.out 4 "[DEBUG] proof_b : [%a]@." Print.pp_term new_proof;
             ...
             let elim_hyp = fun pb alpha ->
               ...
               Console.out 4 "[DEBUG] MAP U v V : "; Extra.IntMap.iter (fun s t -> Console.out 4 ", %d => [%a]@." s (List.pp Print.pp_term "; ") t)  new_mu;
           ...
           match Basics.get_args (unfold proof') with
           |Vari(_), []    ->
           ...
           |_      -> Console.out 1 "Unhandled case : [%a]@.[%a]@." Print.pp_term formula Print.pp_term proof;
                      assert false
      end
    else
