...
let output : string -> Sign.t -> term -> string -> string -> unit =
    ...
    let file_name =
      (Filename.remove_extension file_name) ^ "_deskolemized.lp"  in
    ...
    let f _ s =
      if is_axiom s.sym_name cfg.symb_Skolemized.sym_name then
        Common.Console.out 1 "constant symbol %s : %a;@."
          s.sym_name Print.term !(s.sym_type)
    in
    Extra.StrMap.iter f !(sign.sign_symbols);
    Common.Console.out 1 "@.";
    (* print the original axiom. *)
    Common.Console.out 1 "// The original axiom before skolemization@.";
    Common.Console.out 1 "constant symbol %s : %a;@."
      cfg.symb_Axiom.sym_name Print.term !(cfg.symb_Axiom.sym_type);
    Common.Console.out 1 "@.";
